...
int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  id_x = idx;
  if(id_x+*Len <= APP_RX_DATA_SIZE)
  {
    memcpy((void *)&UserRxBufferFS[id_x], Buf, (*Len));
  }
  else
  {
    memcpy((void *)&UserRxBufferFS[id_x], Buf, APP_RX_DATA_SIZE-id_x);
    memcpy((void *)&UserRxBufferFS[0], &Buf[APP_RX_DATA_SIZE-id_x], (*Len)-(APP_RX_DATA_SIZE-id_x));
  }


  idx = (idx + (*Len)) % (APP_RX_DATA_SIZE);
...
int8_t USB_getchar()
{
  ...
  c=UserRxBufferFS[iusb];
  iusb = (iusb + 1)% (APP_RX_DATA_SIZE);
